...
import config
import data_manager
from online_mf import online_pca_score
...
def main():
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        print(f"\n=== Universe: {universe_name} (Online MF) ===")
        ...
        for win in config.WINDOWS:
            ...
            scores = online_pca_score(returns, win,
                                      n_components=config.N_COMPONENTS,
                                      learning_rate=config.LEARNING_RATE,
                                      forget_factor=config.FORGETTING_FACTOR)
            if scores is None:
                continue
        ...
        top_etfs = [{"ticker": ticker, "pca_loading": float(score), "best_window": win} for ticker, (score, win) in sorted_etfs[:config.TOP_N]]

        print(f"  Top 3 ETFs by incremental PCA loading: {[e['ticker'] for e in top_etfs]}")
